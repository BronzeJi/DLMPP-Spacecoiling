function [alpha]=Optimization_Fano(x)
    %equal opening
    
    f=linspace(1,2000,30000);
    f_resonance=300;
    %f_target=[f_resonance,f_resonance*2,f_resonance*4];%f_resonance*6,f_resonance*8,f_resonance*10,f_resonance*12,f_resonance*14,f_resonance*16];
    %parameter
    a=60*10^(-3);
    % b=15*10^(-3);
    % h=40*10^(-3);
...
%     tic;
    ...
    elapsed_time = toc;
%     disp(['Elapsed time1: ', num2str(elapsed_time), ' seconds']);
%     tic;
    %Transfer matrix
...
%   elapsed_time = toc;
%     disp(['Elapsed time2: ', num2str(elapsed_time), ' seconds']);
%     tic;
...
%         Pvector0(:,i)=M0\[0;0;1;1;0;0];
    ...
    % Stop the timer and display the elapsed time
    elapsed_time = toc;
    disp(['Elapsed time3: ', num2str(elapsed_time), ' seconds']);
% 
